...
//   });
    ...
    // แปลงจากวินาทีเป็นนาที
    Object.keys(losstimeData).forEach(date => {
        losstimeData[date] = losstimeData[date] / 60; // แปลงเป็นนาที
    });
...
async function updateCharts() {
  ...
  losstimeChart = new Chart(document.getElementById('losstimeChart'), {
    ...
    options: {
        ...
        scales: {
            ...
            y: { title: { display: true, text: 'Losstime (minutes)', color: '#00d1ff', font: { size: 14, weight: 'bold' } } }
        },
        plugins: {
            datalabels: {
                display: true,
  ...
  alarmChart = new Chart(document.getElementById('alarmChart'), {
      ...
      options: {
          ...
          plugins: {
              datalabels: {
                  display: true,
                  align: 'top',
                  color: '#ff6384', // สีของตัวเลขที่แสดงบนหัวกราฟ
                  font: {
                      weight: 'bold',
                      size: 12
                  },
      ...
      plugins: [ChartDataLabels] // เพิ่ม ChartDataLabels เป็น plugins ของ Chart
  });
}
